...
export class CompanyController {
  ...
  @UsePipes(new ValidationPipe({ whitelist: true, forbidNonWhitelisted: true }))
  @UseInterceptors(AnyFilesInterceptor())   // ✅ AJOUT
  async createOrUpdateInvoiceConfiguration(
    @Req() req: Request,
    @Param('companyId') companyId: string,
    @Body() dto: CreateInvoiceConfigurationDto,
    @CurrentUser() user: UserEntity,
    @UploadedFiles() files: Express.Multer.File[],   // ✅ AJOUT
  ) {
    const lang = this.extractLanguage(req);
  ...
  async updateInvoiceConfiguration(
    ...
    return this.companyService.updateInvoiceConfiguration(
      companyId,
      dto,
      lang,
      logoFile,        // ✅ passer le fichier
