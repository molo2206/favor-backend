...
export class CompanyService {
  ...
  async setUserCompanyPermissions(
    ...
    // ============================================================
    // 🔥 3. VÉRIFIER LA BRANCHE (si fournie)
    // ============================================================
    ...
    if (branchId) {
      ...
      console.log('ℹ️ Aucune branche fournie');
    }

    // ============================================================
    // 🔥 4. RÉCUPÉRER LES PERMISSIONS AUTORISÉES PAR LA COMPANY
    // ============================================================
    ...
    const allowedResourceIds = new Set(
      companyResources.map((r) => r.resourceId),
    );

    console.log(
      `🔐 Permissions autorisées au niveau de la company (${companyId}):`,
      Array.from(allowedResourceIds),
    );

    // ============================================================
    // 🔥 5. FILTRER LES PERMISSIONS SÉLECTIONNÉES PAR L'ADMIN
    //        → garder UNIQUEMENT celles autorisées par la company
    // ============================================================
    const filteredPermissions = permissions.filter((perm) =>
      allowedResourceIds.has(perm.resourceId),
    );

    const ignoredPermissions = permissions.filter(
      (perm) => !allowedResourceIds.has(perm.resourceId),
    );

    if (ignoredPermissions.length > 0) {
      console.warn(
        `⚠️ Permissions ignorées car non autorisées au niveau de la company:`,
        ignoredPermissions.map((p) => p.resourceId),
      );
    }

    console.log(
      `✅ Permissions à attribuer: ${filteredPermissions.length} / ${permissions.length}`,
    );

    // ============================================================
    // 🔥 6. VÉRIFIER OU CRÉER user_has_company
    // ============================================================
    let userCompany = await this.userHasCompanyRepository.findOne({
      ...
      relations: ['user', 'company', 'branch'],
    });

    const wasAlreadyAssigned = !!userCompany;
    ...
    if (!userCompany) {
      ...
      console.log('✅ user_has_company créé (affectation automatique):', userCompany.id);
    } else {
      console.log('✅ user_has_company existant:', userCompany.id);
      if (branch) {
        userCompany.branch = branch;
        userCompany.branchId = branchId!;
        userCompany = await this.userHasCompanyRepository.save(userCompany);
        console.log('✅ Branche mise à jour pour user_has_company');
      }
    }

    // ============================================================
    // 🔥 7. SUPPRIMER TOUTES LES ANCIENNES PERMISSIONS DE CET UTILISATEUR
    //        (toutes branches confondues pour cette company)
    // ============================================================
    const deleteResult = await this.companyUserResourceRepo.delete({
      userCompanyId: userCompany.id,
    });
    console.log(
      `🗑️ Anciennes permissions supprimées (user: ${userId}): ${deleteResult.affected || 0} lignes`,
    );

    // ============================================================
    // 🔥 8. INSÉRER UNIQUEMENT LES PERMISSIONS FILTRÉES
    // ============================================================
    ...
    // ============================================================
    // 🔥 9. METTRE À JOUR activeCompanyId ET activeBranchId
    // ============================================================
    ...
    if (branch && user.activeBranchId !== branchId) {
    ...
    console.log(
      `🎉 ${insertedPermissions.length} permissions enregistrées pour l'utilisateur ${userId}`,
    );
    console.log(
      `🎉 Statut affectation: ${wasAlreadyAssigned ? 'DÉJÀ affecté → permissions remplacées' : 'NOUVELLEMENT affecté → permissions attribuées'}`,
    );
  ...
  async createOrUpdateInvoiceConfiguration(
    ...
    lang: string = 'fr',
    logoFile?: Express.Multer.File,   // ✅ AJOUT
    ...
    if (logoFile) {
      const uploadResult = await this.filesService.uploadFile(
        logoFile,
        'invoice-configuration/logo',
        'default',
      );
      logoUrl = uploadResult.data as string;
    }

    // 3. Chercher la config existante
    ...
    if (config) {
      ...
      // ✅ Si un nouveau logo est uploadé, on remplace
      if (logoUrl) {
        config.logo = logoUrl;
      }
    ...
    const newConfig = this.invoiceConfigRepo.create({
      companyId,
      logo: logoUrl ?? dto.logo ?? null,   // ✅ logo uploadé en priorité
  ...
  async updateInvoiceConfiguration(
    ...
    if (logoFile) {
      ...
      config.logo = uploadResult.data as string;
    }
