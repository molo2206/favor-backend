...
async sendVehicleRentalPdf(
  ...
  // ✅ Traductions pré-calculées par l'appelant
  const t = context.translations;

  // ✅ Pas de invoiceConfig : le template utilisera le logo par défaut
  const pdfBuffer = await this.generatePdfFromTemplate(
    'vehicle-rental-invoice.ejs',
    {
      user,
      reservation,
      lang,
  ...
  await this.mailerService.sendMail({
    ...
    attachments: [
      {
        filename: `facture-location-${reservation.id.slice(0, 8)}.pdf`,
        content: pdfBuffer,
        contentType: 'application/pdf',
      },
    ],
