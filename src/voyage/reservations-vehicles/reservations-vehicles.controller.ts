...
export class ReservationsController {
  ...
  async myReservations(
    ...
    console.log('🎯 ========== DEBUG myReservations ==========');
    console.log('🎯 user.id:', user.id);
    console.log('🎯 query.page:', page);
    console.log('🎯 query.limit:', limit);
    console.log('🎯 query.type (brut):', type);
    console.log('🎯 typeof type:', typeof type);
    console.log('🎯 URL complète:', req.url);

    const typeEnum =
      type && Object.values(ReservationType).includes(type as ReservationType)
        ? (type as ReservationType)
        : undefined;

    console.log('🎯 typeEnum final:', typeEnum);
    console.log('🎯 =========================================');

    const data = await this.reservationsService.findAllByUser(
      user.id,
      +page,
      +limit,
      typeEnum,
    ...
    return {
      message: await this.i18n.translate('reservation.my_reservations', lang),
      data,
    };
  }
  ...
  async findByTrip(
    ...
    return this.reservationsService.findAllByTrip(tripId, +page, +limit, typeEnum, lang);
  }
