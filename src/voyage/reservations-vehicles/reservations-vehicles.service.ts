...
import { ReservationType, ReservationVehicule } from './entities/reservations-vehicle.entity';
...
export class ReservationsVehiclesService {
  ...
  private async createVehicleReservation(
    ...
    // 1. Vérifier productId
    if (!createDto.productId) {
      throw new BadRequestException(
        await this.i18n.translate('reservation.error.product_id_required', lang),
      );
    }
    ...
    if (!product) {
      throw new NotFoundException(
        await this.i18n.translate('reservation.error.product_not_found', lang, {
    ...
    if (!createDto.startDate || !createDto.endDate) {
      throw new BadRequestException(
        await this.i18n.translate('reservation.error.dates_required', lang),
      );
    }
    ...
    if (startDate >= endDate) {
      throw new BadRequestException(
        await this.i18n.translate('reservation.error.invalid_dates', lang),
      );
    }
    ...
    const conflict = await this.reservationRepository
      ...
      .andWhere('r.type = :type', { type: ReservationType.VEHICLE })
    ...
    if (conflict > 0) {
      throw new BadRequestException(
        await this.i18n.translate('reservation.error.vehicle_not_available', lang),
      );
    }
    ...
    // ============================================================
    if (
      createDto.paymentMethod !== PaymentMethod.MOBILE_MONEY &&
      createDto.paymentMethod !== PaymentMethod.FPAY
    ) {
      throw new BadRequestException(
        await this.i18n.translate('reservation.error.payment_method_invalid', lang),
      );
    }
    ...
    // ============================================================
    const totalAmount = createDto.totalAmount ?? 0;
    const currency = createDto.currency || 'USD';

    let paymentStatus = ReservationStatus.PENDING;
    let isPaid = false;
    let fpayTransactionId: string | null = null;
    let fpayReference: string | null = null;
    const selectedMethod: PaymentMethod = createDto.paymentMethod!;
    ...
    if (userId) {
      ...
      console.log(`🔍 ========== DEBUG PARRAINAGE VÉHICULE ==========`);
      console.log(`🔍 userId: ${userId}`);
      console.log(`🔍 referredBy: ${userWithReferrer?.referredBy}`);
      console.log(`🔍 referrer.id: ${userWithReferrer?.referrer?.id || 'null'}`);
      console.log(`🔍 referrer.fullName: ${userWithReferrer?.referrer?.fullName || 'null'}`);
      console.log(`🔍 ================================================`);

      hasReferrer = !!(userWithReferrer?.referrer);
      referrerId = userWithReferrer?.referrer?.id ?? null;
    }

    // ✅ Parrainage : 5% du montant total
    let parrainageAmount = 0;
    if (hasReferrer && totalAmount > 0) {
      parrainageAmount = totalAmount * 0.05;

      console.log(`[VehicleReservation] 👤 A un parrain: ${hasReferrer}`);
      console.log(`[VehicleReservation] Total: ${totalAmount}$`);
      console.log(`[VehicleReservation] 5% parrainage: ${parrainageAmount}$`);
    } else {
      console.log(`[VehicleReservation] ℹ️ Aucun parrain ou montant nul → pas de parrainage`);
    }

    // ============================================================
    // ✅ MOBILE MONEY (Pawapay) — OBLIGATOIRE
    // ============================================================
    if (createDto.paymentMethod === PaymentMethod.MOBILE_MONEY) {
      if (!createDto.mobileMoneyDetails?.providerId || !createDto.mobileMoneyDetails?.phone) {
        throw new BadRequestException(
          await this.i18n.translate('order.mobile_money_provider_phone_required', lang),
        );
      }

      const phone = createDto.mobileMoneyDetails.phone.trim();
      if (!phone) {
        throw new BadRequestException(
          await this.i18n.translate('order.mobile_money_invalid_phone', lang),
        );
      }

      if (!totalAmount || totalAmount <= 0) {
        throw new BadRequestException(
          await this.i18n.translate('order.mobile_money_grandtotal_required', lang),
        );
      }
      ...
      console.log('[VehicleReservation] Création dépôt Pawapay :', pawapayData);

      try {
        const pawapayResponse = await this.pawapayService.createDepositSimple(pawapayData);
        console.log('[VehicleReservation] Réponse Pawapay :', JSON.stringify(pawapayResponse, null, 2));
        ...
        console.log(`[VehicleReservation] Statut final Pawapay: ${depositStatus}`);

        if (depositStatus === 'COMPLETED') {
          console.log('[VehicleReservation] ✅ Dépôt Pawapay confirmé : COMPLETED');
          paymentStatus = ReservationStatus.CONFIRMED;
          isPaid = true;
          ...
          throw new BadRequestException({
            ...
            message:
              ...
              `Le paiement est en cours de traitement (${lastStatus}). Veuillez vérifier plus tard.`,
          ...
          console.log(`[VehicleReservation] Statut en attente: ${depositStatus}`);
          paymentStatus = ReservationStatus.PENDING;
          isPaid = false;
        } else {
          throw new BadRequestException({
            statusCode: HttpStatus.BAD_REQUEST,
        ...
        // ✅ Tentative FPAY en parallèle (bonus, non bloquant)
        if (isPaid) {
          try {
            const fpayResponse = await this.fpayService.payWithMobileMoney(
              totalAmount,
              currency,
              `Paiement réservation véhicule ${product.name}`,
              'MOBILE_MONEY',
              lang,
            );
            ...
            if (fpayResponse?.data?.transaction?.status === 'SUCCESS') {
              ...
              console.log('[VehicleReservation] ✅ FPAY Mobile Money réussi');
            }
          } catch (error: any) {
            console.log('[VehicleReservation] FPAY Mobile Money ignoré:', error.message);
          }
        }
      } catch (error: any) {
        if (error instanceof BadRequestException) throw error;
        console.error('[VehicleReservation] Erreur Pawapay:', error.message);
        throw new BadRequestException(
          error.message || (await this.i18n.translate('order.payment_failed', lang)),
        );
      }
    }
    ...
    // ✅ FPAY — OBLIGATOIRE
    // ============================================================
    else if (createDto.paymentMethod === PaymentMethod.FPAY) {
      if (!createDto.access_token) {
        throw new BadRequestException(
          await this.i18n.translate('reservation.fpay_token_required', lang),
        );
      }
      ...
      const fpayData = {
        amount: totalAmount,
        currency,
        description: `Paiement réservation véhicule ${product.name}`,
      ...
      try {
        ...
        if (fpayResponse?.data?.transaction?.status === 'SUCCESS') {
          ...
          console.log('[VehicleReservation] ✅ Paiement FPAY réussi:', {
        ...
        console.error('[VehicleReservation] ❌ Erreur FPAY:', error.message);
        throw new BadRequestException(
          error.message || (await this.i18n.translate('order.payment_failed', lang)),
        );
      }
    ...
    try {
      const reservation = this.reservationRepository.create({
        ...
        transactionFee: createDto.transactionFee,
        status: paymentStatus,
        currency: currency,
      } as Partial<ReservationVehicule>);
      ...
      if (isPaid) {
        const operationData: any = {
          debit: 0,
          credit: totalAmount,
          designation: `Paiement réservation véhicule #${savedReservation.id.slice(0, 8)}`,
        ...
        if (selectedMethod === PaymentMethod.MOBILE_MONEY && createDto.mobileMoneyDetails?.providerId) {
      ...
      const full = await this.reservationRepository.findOne({
        ...
        relations: ['product', 'product.company', 'user'],
      });

      if (full?.user) {
        const { password, ...rest } = full.user;
        full.user = rest as any;
      }

      // ============================================================
      // ✅ ENVOYER LE PARRAINAGE (5%) - UNIQUEMENT SI PARRAIN + PAIEMENT CONFIRMÉ
      // ============================================================
      if (full?.user) {
        const { password, ...rest } = full.user;
        full.user = rest as any;
      ...
      if (hasReferrer && parrainageAmount > 0 && paymentStatus === ReservationStatus.CONFIRMED && full && full.user) {
        console.log(`[VehicleReservation] 👤 Parrain détecté`);
        console.log(`[VehicleReservation] 5% parrainage: ${parrainageAmount}$`);

        const reservationRef = savedReservation.id.slice(0, 8);

        const sendDto: FpaySendDto = {
          amount: parrainageAmount,
          description: `Bonus parrainage (5%) - Réservation véhicule de votre filleul, réf #${reservationRef}`,
        ...
        this.fpayService.makeSendparrainage(sendDto, full.user)
          .then(async (result) => {
            if (result && result.success) {
              console.log(`[VehicleReservation] ✅ ${parrainageAmount} envoyé au compte Parrainage via FPay`);
              ...
              console.log(`[VehicleReservation] ⚠️ Parrainage non envoyé: ${result.message}`);
            }
          })
          .catch(error => {
            console.error(`[VehicleReservation] ❌ Erreur envoi parrainage:`, error.message);
          });

        console.log(`[VehicleReservation] 📤 Envoi parrainage en cours (non bloquant)...`);
      }
      // ⏭️ Aucun traitement si paiement non confirmé

      // ✅ Notifications (non bloquant, même pattern que create())
      // ✅ Notifications véhicule (non bloquant)
      if (userId && full) {
        this.processVehicleRentalNotifications(
          full,
          full.user,
          totalAmount,
          selectedMethod,
          lang,
        ).catch(err => console.error('Erreur notifications véhicule:', err));
      }
  ...
  async create(createDto: CreateReservationDto, userId?: string, lang: string = 'fr'): Promise<any> {
    ...
    // ============================================================
    // ✅ REDIRECTION VERS VEHICLE
    // ============================================================
    if (createDto.type === ReservationType.VEHICLE) {
      return this.createVehicleReservation(createDto, userId, lang);
    }

    // ============================================================
    // ✅ VÉRIFICATION passengers (obligatoire pour TRIP)
    // ============================================================
    if (!createDto.passengers || !Array.isArray(createDto.passengers) || createDto.passengers.length === 0) {
      throw new BadRequestException(
        await this.i18n.translate('reservation.error.passengers_required', lang),
      );
    }

    const passengers = createDto.passengers;

    // ⬇️⬇️⬇️ CODE EXISTANT ⬇️⬇️⬇️
    ...
    const allSeatIds = [...new Set(passengers.flatMap(p => p.seats.map(s => s.seatId)))];
    ...
    if (hasSegments && passengers.length > 0) {
      for (const passenger of passengers) {
        for (const seatRes of passenger.seats) {
  ...
  private async processVehicleRentalNotifications(
    reservation: ReservationVehicule,
    user: UserEntity,
    totalAmount: number,
    ...
    // ✅ Product : fallback sur un objet vide si non chargé (jamais null)
    const productName = reservation.product?.name ?? 'Véhicule';
    ...
    if (isConfirmed) {
      pushTitle = await this.i18n.translate('reservation.push.vehicle_confirmed_title', lang);
      pushBody = await this.i18n.translate('reservation.push.vehicle_confirmed_body', lang, {
        ref: reservationRef, total: finalTotal, currency, vehicle: productName,
      });
      smsBody = await this.i18n.translate('reservation.sms.vehicle_confirmed_body', lang, {
        ref: reservationRef, total: finalTotal, currency, vehicle: productName,
      });
    } else {
      pushTitle = await this.i18n.translate('reservation.push.vehicle_pending_title', lang);
      pushBody = await this.i18n.translate('reservation.push.vehicle_pending_body', lang, {
        ref: reservationRef, total: finalTotal, currency, vehicle: productName,
      });
      smsBody = await this.i18n.translate('reservation.sms.vehicle_pending_body', lang, {
        ref: reservationRef, total: finalTotal, currency, vehicle: productName,
      });
    }
    ...
    // ============================================================
    // 📧 EMAIL — Facture location véhicule
    // ============================================================
    if (hasEmail) {
      try {
        const subject = await this.i18n.translate(
          'reservation.email.vehicle_invoice_subject',
          lang,
          { ref: reservationRef },
        );

        const emailBody = await this.i18n.translate(
          'reservation.email.vehicle_invoice_body',
          lang,
          {
            name: user.fullName,
            vehicle: productName,
        ...
        const translations = {
          title: await this.i18n.translate('vehicle_invoice.title', lang),
          notes_title: await this.i18n.translate('vehicle_invoice.notes_title', lang),
          note_1: await this.i18n.translate('vehicle_invoice.note_1', lang),
          note_2: await this.i18n.translate('vehicle_invoice.note_2', lang),
          note_3: await this.i18n.translate('vehicle_invoice.note_3', lang),

          vehicle: await this.i18n.translate('vehicle_invoice.vehicle', lang),
          start_date: await this.i18n.translate('vehicle_invoice.start_date', lang),
          end_date: await this.i18n.translate('vehicle_invoice.end_date', lang),
          duration: await this.i18n.translate('vehicle_invoice.duration', lang),
          days: await this.i18n.translate('vehicle_invoice.days', lang),
          rental_service: await this.i18n.translate('vehicle_invoice.rental_service', lang),

          number: await this.i18n.translate('vehicle_invoice.number', lang),
          date: await this.i18n.translate('vehicle_invoice.date', lang),
          designation: await this.i18n.translate('vehicle_invoice.designation', lang),
          quantity: await this.i18n.translate('vehicle_invoice.quantity', lang),
          unit_price: await this.i18n.translate('vehicle_invoice.unit_price', lang),
          amount: await this.i18n.translate('vehicle_invoice.amount', lang),
          payment_method: await this.i18n.translate('vehicle_invoice.payment_method', lang),
          cash: await this.i18n.translate('vehicle_invoice.cash', lang),
          total_ht: await this.i18n.translate('vehicle_invoice.total_ht', lang),
          vat: await this.i18n.translate('vehicle_invoice.vat', lang),
          total_ttc: await this.i18n.translate('vehicle_invoice.total_ttc', lang),
          status_paid: await this.i18n.translate('vehicle_invoice.status_paid', lang),
          status_unpaid: await this.i18n.translate('vehicle_invoice.status_unpaid', lang),
          thank_you: await this.i18n.translate('vehicle_invoice.thank_you', lang),
          legal_line_1: await this.i18n.translate('vehicle_invoice.legal_line_1', lang),
          legal_line_2: await this.i18n.translate('vehicle_invoice.legal_line_2', lang),
          legal_line_3: await this.i18n.translate('vehicle_invoice.legal_line_3', lang),
        };

        // ✅ Plus de invoiceConfig transmis
        await this.mailOrderService.sendVehicleRentalPdf(
          user.email,
          subject,
          {
            user,
        ...
        console.log(`✅ Email facture véhicule envoyé à ${user.email}`);
      } catch (emailError) {
        console.error('❌ Erreur envoi email véhicule:', emailError);
      }
    }
    ...
    try {
      ...
      console.error('❌ Erreur notification in-app véhicule:', notifError);
    }
    ...
    try {
      ...
      console.error('❌ Erreur notification super admins véhicule:', adminError);
    }

    // ============================================================
    // ✅ CORRECTION : Propriétaires de la compagnie du produit
    // company_id n'existe pas sur Product → utiliser company?.id
    // ============================================================
    const companyId = reservation.product?.company?.id ?? null;
    if (companyId) {
      try {
        const companyAdmins = await this.userHasCompanyRepo.find({
          where: {
        ...
        console.error('❌ Erreur notification entreprise véhicule:', companyError);
      }
    }

    console.log(`✅ Notifications véhicule envoyées pour la réservation ${reservation.id}`);
  }
  // ==================== CRÉATION PAR ADMIN ====================
  async createByAdmin(
    ...
    // ============================================================
    // ✅ VÉRIFICATION passengers (obligatoire)
    // ============================================================
    if (!createDto.passengers || !Array.isArray(createDto.passengers) || createDto.passengers.length === 0) {
      throw new BadRequestException(
        await this.i18n.translate('reservation.error.passengers_required', lang),
  ...
  async findAllByUser(
    ...
    console.log('🔍 Types retournés:', data.map(d => d.type));
    console.log('🔍 Total en DB (avec filtre):', total);
  ...
  async findAllByCompany(
    ...
    const where: any = { trip: { company_id: companyId } };
