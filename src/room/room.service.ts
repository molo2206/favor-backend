  ...
  async deleteFromCloudinary(fileUrls: string | string[]): Promise<{ message: string }> {
    ...
    try {
      if (Array.isArray(fileUrls)) {
        await Promise.all(fileUrls.map((url) => this.cloudinary.handleDeleteFile(url)));
        return { message: 'Tous les fichiers ont été supprimés avec succès.' };
      } else {
        await this.cloudinary.handleDeleteFile(fileUrls);
