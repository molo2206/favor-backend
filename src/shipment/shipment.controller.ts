...
export class ShipmentController {
  ...
  async updateShipmentAdmin(
    ...
    const lang = this.extractLanguage(req);
    const currentUser = req.user as UserEntity;  // ⬅️ récupérer l'admin connecté
    return this.shipmentService.updateByAdmin(id, dto, file, currentUser, lang);
  }

  @Get('invoice/pdf/:trackingNumber')
  async getShipmentInvoicePdf(
    @Param('trackingNumber') trackingNumber: string,
    @Req() req: Request,
    @Res() res: Response,
  ) {
    const lang = this.extractLanguage(req);
    const { pdfBuffer } = await this.shipmentService.generateShipmentInvoiceByTrackingNumber(
      trackingNumber,
      lang,
    );
